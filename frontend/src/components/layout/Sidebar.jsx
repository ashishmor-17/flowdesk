    ...
    <aside className="glass-panel" style={{ width: '280px', height: '100vh', display: 'flex', flexDirection: 'column', padding: '24px 16px', borderRadius: '0 24px 24px 0', borderLeft: 'none' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '32px', paddingLeft: '8px' }}>
        ...
        <div>
          ...
          <span style={{ fontSize: '11px', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.1em' }}>Work Management Platform</span>
        </div>
      </div>
      ...
      <nav style={{ display: 'flex', flexDirection: 'column', gap: '8px', flexGrow: 1 }}>
        ...
        <button
          ...
          style={{
            display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', border: 'none', borderRadius: '10px', cursor: 'pointer', fontSize: '15px', fontWeight: '500', transition: 'all 0.2s', width: '100%',
            background: activeTab === 'settings' ? 'linear-gradient(135deg, rgba(99, 102, 241, 0.15), rgba(168, 85, 247, 0.15))' : 'transparent',
            color: activeTab === 'settings' ? 'var(--text-primary)' : 'var(--text-secondary)',
            boxShadow: activeTab === 'settings' ? 'inset 0 0 0 1px var(--border-glass-active)' : 'none'
          }}
        >
          <Settings size={18} /> Settings
        </button>

        {(user?.role?.toUpperCase() === 'OWNER' || user?.role?.toUpperCase() === 'ADMIN') && (
          <button
            onClick={() => setActiveTab('admin')}
            style={{
              display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', border: 'none', borderRadius: '10px', cursor: 'pointer', fontSize: '15px', fontWeight: '500', transition: 'all 0.2s', width: '100%',
              background: activeTab === 'admin' ? 'linear-gradient(135deg, rgba(99, 102, 241, 0.15), rgba(168, 85, 247, 0.15))' : 'transparent',
              color: activeTab === 'admin' ? 'var(--text-primary)' : 'var(--text-secondary)',
              boxShadow: activeTab === 'admin' ? 'inset 0 0 0 1px var(--border-glass-active)' : 'none'
            }}
          >
